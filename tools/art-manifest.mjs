...
p('| Asset | Count | State |');
p('| --- | --- | --- |');
p(`| Food icons, finished | ${drawn.length} | done — original and commissioned drawings |`);
p(`| Food icons, script-drawn | ${generated.length} | ${generated.length ? 'usable, worth replacing by hand' : 'none'} |`);
...
p(PROMPT);
p('');
p(`### Remaining art queue (${generated.length + missing.length})`);
p('');
p(generated.length || missing.length
  ? 'These are aisle fallbacks drawn by `tools/make-food-icons.mjs`. They are consistent and read at 24 px, but still need an ingredient-specific art pass.'
